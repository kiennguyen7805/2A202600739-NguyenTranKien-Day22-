...
MAX_STEPS = int(os.environ.get("DPO_MAX_STEPS", "-1"))  # >0 = cap số step (tiết kiệm thời gian)
...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    dtype=None,
    load_in_4bit=True,
    attn_implementation="sdpa",   # xformers backward không hỗ trợ GQA BMGHK → dùng SDPA
)
...
dpo_config = DPOConfig(
    ...
    max_steps=MAX_STEPS,           # >0 overrides epochs (cap thời gian trên L4)
    learning_rate=LR,
    beta=BETA,
    max_length=MAX_LEN,
    ...
    # W&B rigor add-on (+2): logs reward curves if WANDB_API_KEY is set, else off.
    report_to="wandb" if os.environ.get("WANDB_API_KEY") else "none",
    run_name=f"dpo-beta{BETA}-{COMPUTE_TIER}",
)
